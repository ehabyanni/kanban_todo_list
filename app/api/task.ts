...
export const getTasksByColumn = async (columnId: string, pageParam: number, searchTerm: string = "") => {
  const limit = 4;
  let url = `${API_URL}?column=${columnId}&_page=${pageParam}&_limit=${limit}`;
  
  if(searchTerm){
    url += `&title_like=${searchTerm}&description_like=${searchTerm}`;
  }
